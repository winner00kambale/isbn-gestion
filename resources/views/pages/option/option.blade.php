                ...
                <div class="card mb-3">
                    <div class="card-header">
                        <div class="d-flex justify-content-between align-items-center">
                            <a href="{{ route('option.form') }}" class="btn btn-outline-primary btn-lg">
                                <i class="bi bi-person"></i> Ajouter
                            </a>
                            <div class="d-flex align-items-center gap-2">

                                <form id="search-form" action="{{ route('option.index') }}" method="GET" class="d-flex">
                        ...
                        <div class="table-responsive">
                                            ...
                                            <tr class="grd-primary-light">
                                                ...
                                                <td>
                                                    <a class="btn btn-info btn-sm"
                                                        href="{{ route('option.edit', ['option' => $option->id]) }}">
                            ...
                            @if ($options->lastPage() > 1)
                                <nav aria-label="Page Navigation">
                                    <ul class="pagination">
                                        <!-- Lien "Previous" -->
                                        <li class="page-item {{ $options->currentPage() == 1 ? 'disabled' : '' }}">
                                        ...
                                        @for ($i = 1; $i <= $options->lastPage(); $i++)
                                            <li class="page-item {{ $options->currentPage() == $i ? 'active' : '' }}">
                                                <a class="page-link" href="{{ $options->url($i) }}">
                                                    {{ $i }}
                                                </a>
                                            </li>
                                        @endfor
                                        ...
                                        <!-- Lien "Next" -->
                                        <li
                                            class="page-item {{ $options->currentPage() == $options->lastPage() ? 'disabled' : '' }}">
                                            <a class="page-link" href="{{ $options->nextPageUrl() }}" aria-label="Next">
                                                <span aria-hidden="true">
